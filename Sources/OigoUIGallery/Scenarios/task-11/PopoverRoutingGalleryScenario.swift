...
private final class SyntheticStatusSurface: NSObject {
    ...
    func install() {
        ...
        fflush(stdout)
        teardownTimer = Timer.scheduledTimer(withTimeInterval: 45, repeats: false) { [weak self] _ in
            MainActor.assumeIsolated {
                self?.teardown()
            }
        }
    ...
    func probeInactivePrimaryRoute() {
        togglePopover()
        popover.close()
        activateForegroundSentinel { [weak self] sentinelActivated in
            DispatchQueue.main.async {
                self?.recordDismissedActivation(sentinelActivated: sentinelActivated)
                self?.recordHostObservation()
            }
        }
    }
    ...
    func activateForegroundSentinel(completion: @escaping (Bool) -> Void) {
        ...
        if !requested {
            finishForegroundSentinelActivation(activated: !NSApp.isActive)
            return
        }
        DispatchQueue.main.async { [weak self] in
            guard let self, self.activationCompletion != nil, !NSApp.isActive else {
                return
            }
            self.finishForegroundSentinelActivation(activated: true)
        }
    }
    ...
    func recordDismissedActivation(sentinelActivated: Bool) {
        routeGeneration &+= 1
        let passed = sentinelActivated
            && NSApp.activationPolicy() == .accessory
            && !NSApp.isActive
        record(
            "route=primary-dismissed generation=\(routeGeneration) popover=closed menu=closed verification="
                + (passed ? "pass " : "fail ")
                + "foreground-handoff=" + (sentinelActivated ? "observed " : "unobserved ")
                + activationReceipt()
        )
    }
    ...
    private func showMenu(route: String, relativeTo routeView: NSView? = nil) {
        ...
        routeGeneration &+= 1
        record(
            "route=\(route) generation=\(routeGeneration) popover=closed menu=open entries=History,Settings,separator,Quit "
                + activationReceipt()
        )
        menu.popUp(
            positioning: nil,
