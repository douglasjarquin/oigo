...
public final class DictationCoordinator {
    ...
    private var transcriptionCancellationTask: (
        operationID: UUID,
        task: Task<TranscriptionResult?, any Error>
    )?
    ...
    private func startRecordingWithTranscriptionInternal(
        ...
        do {
            ...
            capture.cancel()
            _ = try? await requestCancellation(
                transcription,
                operationID: operationID,
                stage: .cancellation
            )
    ...
    public func stopRecordingWithTranscription(
        ...
        do {
            ...
            let result = try await withTaskCancellationHandler(operation: {
                ...
                Task { @MainActor [weak self] in
                    _ = try? await self?.requestCancellation(
                        transcription,
                        operationID: operationID,
                        stage: .cancellation
                    )
    ...
    public func finishInsertion(
        ...
        return terminalizeCompletedInsertion(
            session: session,
            store: sessionStore,
            outcome: outcome,
            reason: reason,
    ...
    private func existingCompletedInsertion() -> DictationSession? {
        guard state == .complete,
              let session = currentSession,
    ...
    private func terminalizeCompletedInsertion(
        session: DictationSession,
        store: SessionStore,
        outcome: InsertionOutcome,
        reason: String?,
        ...
        let persisted = persistTerminalState(
            session,
            in: store,
            state: .completed,
            reason: nil,
    ...
    private func cancelAfterRawIfNeeded(reason: String) -> Bool {
        guard [.cleaning, .inserting].contains(state),
              currentSession?.metadata.rawTextByteCount != nil else {
            return false
        }
        _ = failInsertion(reason: reason)
        return true
    ...
    private func finishTranscribedRecording(
        ...
        do {
            ...
            result = try await requestCancellation(
                transcription,
                operationID: operationID,
                stage: stage
            )
    ...
    private func persistTerminalState(
        ...
        do {
            return try store.update(
                ...
                insertionTextSource: insertionTextSource,
                cleanupFallbackReason: cleanupFallbackReason,
            ...
            do {
                ...
                lastTerminalMetadataWriteFailed = true
    ...
    private func requestCancellation(
        ...
        if let cancellation = transcriptionCancellationTask,
           cancellation.operationID == operationID {
            return try await cancellation.task.value
        }
        let cancellation = Task { @MainActor [timeoutPolicy, operationRegistry] in
            try await BoundedOperation.run(
                operationID: operationID,
                stage: stage,
        ...
        transcriptionCancellationTask = (operationID, cancellation)
    ...
    public func shutdown() {
        ...
        activeTask?.cancel()
        activeTask = nil
        if cancelAfterRawIfNeeded(reason: "application shutdown") {
    ...
    public func shutdownWithTranscription() async {
        ...
        _ = cancelAfterRawIfNeeded(reason: "application shutdown")
        diagnostics.record("coordinator shutdown")
    }
}
