...
| Native AppKit launch, onboarding surface, status menu, and idle quit | PASS | `.ulw/evidence/issue10-native-qa.log` and `.ulw/evidence/issue10-native-qa.png` | The temporary QA bundle used an ad hoc local signature and did not complete permission setup. |
...
App-level sleep and workspace-resign-active observers cancel coordinator-owned capture, transcription, cleanup, insertion, and retry work, not only the recorder.
...
The issue #10 contract suite drives cancellation through preparing, recording, finalizing, cleaning, and inserting, including cancellation-aware transcription startup and finalization.
...
The pre-production RED run is bound to live base `ca349411c45b9738983d6793bb2f7136a57faeba` in `.ulw/evidence/issue10-red-bound-ce9d388.log`; the exact-head GREEN run is recorded in `.ulw/evidence/full-regression.log`.
