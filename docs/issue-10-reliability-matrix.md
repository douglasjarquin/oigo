...
| Native AppKit launch, onboarding surface, status menu, and idle quit | PASS | `.ulw/evidence/issue10-native-qa-head-121e8ad.log` and `.ulw/evidence/issue10-native-qa-head-121e8ad.png` | The temporary QA bundle used an ad hoc local signature and did not complete permission setup. |
...
The pre-production RED run is bound to live base `ca349411c45b9738983d6793bb2f7136a57faeba` in `.ulw/evidence/issue10-red-bound-ce9d388.log`; the exact-head GREEN run is recorded in `.ulw/evidence/full-regression-head-121e8ad.log`.
