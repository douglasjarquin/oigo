...
| Audio engine startup failure | PASS | `oigo-issue4-contract-tests` actionable host failure and the recorder startup cleanup path | The test uses a deterministic host failure, not a physical engine fault. |
...
| Transcription startup or analysis failure | PASS | `oigo-issue10-contract-tests` speech fault-injection scenario and `oigo-issue5-contract-tests` analysis failure | Apple speech assets were not available for a real on-device transcription run. |
| Cleanup timeout or unavailable cleanup model | PASS | `oigo-issue10-contract-tests` timeout cleaner and existing cleanup contract coverage | Foundation Models generation was not forced to time out on the host. |
| Target loss before paste | PASS | `oigo-issue10-contract-tests` target-loss fault injection and issue #7 copy-only recovery coverage | An external target process was not terminated during QA. |
| Cancellation during cleanup or insertion preparation | PASS | `oigo-issue10-contract-tests` cancellation terminalization scenario | UI cancellation was exercised through the coordinator fixture, not a live dictation. |
